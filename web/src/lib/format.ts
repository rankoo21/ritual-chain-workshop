...
/** Ritual block.timestamp (milliseconds) -> local date string. */
export function formatTimestamp(unixMillis?: bigint | number): string {
  if (unixMillis === undefined) return "-";
  const ms = Number(unixMillis);
...
/** Compact "in 2h 5m" / "3m ago" style relative label (input is milliseconds). */
export function formatRelative(unixMillis?: bigint | number): string {
  if (unixMillis === undefined) return "";
  const target = Number(unixMillis);
